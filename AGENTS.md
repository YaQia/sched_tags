...
| 标签类型       | 使用场景                                             | 值                            | 调度目标                                                                               |
| -------------- | ---------------------------------------------------- | ----------------------------- | -------------------------------------------------------------------------------------- |
| `exec-dense`   | 执行资源压力大的代码（整数/浮点/SIMD 运算、分支密集） | INT/FLOAT/SIMD/CTRL（或按位或组合） | SMT 配对：执行资源掩码不相交的任务适合作同核兄弟线程（如 SIMD 配分支密集）              |
| `memory-dense` | 内存访问密集代码（大量 load/store）                  | STREAM/RANDOM                 | STREAM 顺序扫描迁移代价低；RANDOM 指针追逐，避免同类配对                               |
...
| 标签类型     | 使用场景             | 值               | 调度目标                                       |
| ------------ | -------------------- | ---------------- | ---------------------------------------------- |
| `load-trend` | 负载即将上升/下降    | RISING / FALLING | RISING 主动抬升 CPU 频率；FALLING 提前降频、进入深 C-state |
...
1. **识别性能关键路径**：
   ...
   - 自旋锁实现 → `atomic-dense`
   - 紧密的计算循环（矩阵运算、图像处理）→ `exec-dense`
   - 大量条件分支的代码 → `exec-dense`（值 `CTRL`）
2. **每个标签必须包含**：
   - `type`：标签类型
