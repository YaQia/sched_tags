 ...
 *         test/test_rust_dense.rs -o /tmp/test_rust_dense_raw.ll
 *
 *   # 2. Strip LLVM-22-only attributes (rustc ships LLVM 22; system opt is 21)
 *   sed 's/nocreateundeforpoison //g' /tmp/test_rust_dense_raw.ll \
 *       > /tmp/test_rust_dense_compat.ll
 *
 *   # 3. Optimise + instrument
 *   opt -O1 -load-pass-plugin=build/pass/libSchedTagPass.so \
 *       -passes=sched-tag /tmp/test_rust_dense_compat.ll \
 *       -o /tmp/tagged_rust.bc
 *
 *   # 4. Compile to object
 *   llc /tmp/tagged_rust.bc -filetype=obj -o /tmp/tagged_rust.o
 *
 *   # 5. Link with this reader
...
static void check(const char *label, uint64_t expect_tags, uint8_t expect_compute,
    ...
    printf("  [%-30s] tags_active=0x%lx  compute_dense=%-5s  %s\n",
           label,
...
int main(void) {
    ...
    check("inside mixed_work (cb)",
          SCHED_TAG_COMPUTE_DENSE, SCHED_COMPUTE_INT | SCHED_COMPUTE_FLOAT,
          observed_tags_active, observed_compute_dense);
    check("after mixed_work (main)",
          0x0, SCHED_COMPUTE_NONE,
          h->tags_active, h->compute_dense);
